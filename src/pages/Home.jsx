            ...
            {testimonials.map((t) => (
              <div key={t.id} className="col-md-3">
                <div className="testimonial-card">
                  <div className="testimonial-head d-flex align-items-center mb-3">
                    <img className="avatar rounded-circle me-3" src={t.img} alt={t.name} width="50" height="50" />
                    <div>
                      <div className="fw-bold">{t.name}</div>
                      <div className="text-warning small">
                        {[1, 2, 3, 4, 5].map((n) => (
                          <i key={n} className=" small-stars fa-solid fa-star"></i>
                        ))}
                      </div>
                    </div>
                  </div>
                  <p className="quote text-muted">"{t.quote}"</p>
                </div>
              </div>
            ))}
          </div>
